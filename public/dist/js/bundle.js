...
module.exports = function (jQuery) {
  ...
  jQuery.extend(F, {
    ...
    open: function open(group, opts) {
      ...
      F.opts = jQuery.extend(true, {}, F.defaults, opts); // All options are merged recursive except keys
    ...
    toggle: function toggle(action) {
      if (F.isOpen) {
        F.current.fitToView = jQuery.type(action) === "boolean" ? action : !F.current.fitToView; // Help browser to restore document dimensions
    ...
    showLoading: function showLoading() {
      var el, viewport;
      F.hideLoading();
      el = jQuery(F.opts.tpl.loading).click(F.cancel).appendTo('body'); // If user will press the escape-button, the request will be canceled
    ...
    _start: function _start(index) {
      ...
      coming = jQuery.extend(true, {}, F.opts, obj); // Convert margin and padding properties to array - top, right, bottom, left
    ...
    _loadIframe: function _loadIframe() {
      var coming = F.coming,
          iframe = jQuery(coming.tpl.iframe.replace(/\{rnd\}/g, new Date().getTime())).attr('scrolling', isTouch ? 'auto' : coming.iframe.scrolling).attr('src', coming.href); // This helps IE
      ...
      if (coming.iframe.preload) {
        ...
        iframe.one('load', function () {
          jQuery(this).data('ready', 1); // iOS will lose scrolling if we resize
...
(0, _jquery2.default)(document).ready(function () {
  ...
  function manageControls(position) {
    ...
    if (position == numberOfSlides - 1) {
      ...
      (0, _jquery2.default)('#rightControl').show();
    }
  }
});
...
var btn = (0, _jquery2.default)('#scrollup');
